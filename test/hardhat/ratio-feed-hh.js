...
describe("RatioFeed", function () {
  ...
  describe("updateRatioBatch(): publishes ratio for listed addresses", function () {
    ...
    it("Reverts: when ratio threshold is not set", async function () {
      await expect(ratioFeed.connect(operator).updateRatioBatch([certificateToken.address], [randomBN(18)]))
    ...
    it("Publish ratio for the first time", async function () {
      const ratioBefore = await ratioFeed.getRatioFor(certificateToken.address);
      console.log(`Ratio before: ${ratioBefore}`);
      const newRatio = e18;
      const tx = await ratioFeed.connect(operator).updateRatioBatch([certificateToken.address], [newRatio]);
      const receipt = await tx.wait();
      const events = receipt.events?.filter((e) => {
        return e.event === "RatioUpdated";
      });
      expect(events.length).to.be.eq(1);
      expect(events[0].args["tokenAddress"]).to.be.eq(certificateToken.address);
      expect(events[0].args["oldRatio"]).to.be.eq(ratioBefore);
      expect(events[0].args["newRatio"]).to.be.eq(newRatio);

      const ratioAfter = await ratioFeed.getRatioFor(certificateToken.address);
    ...
    it("Publish ratio within threshold", async function () {
      ...
      const ratioBefore = await ratioFeed.getRatioFor(certificateToken.address);
      console.log(`Ratio before: ${ratioBefore}`);
      const threshold = ratioBefore.mul(await ratioFeed.getRatioThreshold()).div(await ratioFeed.MAX_THRESHOLD());
    ...
    it("Publish the same ratio", async function () {
      await increaseChainTimeForSeconds(60 * 60 * 12 + 1); //+12h
      const ratioBefore = await ratioFeed.getRatioFor(certificateToken.address);
      const tx = await ratioFeed.connect(operator).updateRatioBatch([certificateToken.address], [ratioBefore]);
      const receipt = await tx.wait();
      const events = receipt.events?.filter((e) => {
        return e.event === "RatioUpdated";
      });
      ...
      expect(await ratioFeed.getRatioFor(certificateToken.address)).to.be.eq(ratioBefore);
    });
    ...
    it("Skips: when publish second time in less than 12h", async function () {
      ...
      expect(events.length).to.be.eq(1);
      expect(events[0].args["tokenAddress"].toLowerCase()).to.be.eq(certificateToken.address.toLowerCase());
      expect(events[0].args["failedRatio"]).to.be.eq(newRatio);
      expect(events[0].args["reason"]).to.be.eq("ratio was updated less than 12 hours ago");
      expect(await ratioFeed.getRatioFor(certificateToken.address)).to.be.eq(ratioBefore);
    });
    ...
    it("Skips: when new ratio > current ratio", async function () {
      ...
      expect(events[0].args["tokenAddress"].toLowerCase()).to.be.eq(certificateToken.address.toLowerCase());
      expect(events[0].args["failedRatio"]).to.be.eq(newRatio);
      expect(events[0].args["reason"]).to.be.eq("new ratio cannot be greater than old");
      expect(await ratioFeed.getRatioFor(certificateToken.address)).to.be.eq(ratioBefore);
    });
    ...
    it("Skips: when current - new ratio > threshold", async function () {
      ...
      expect(events[0].args["tokenAddress"].toLowerCase()).to.be.eq(certificateToken.address.toLowerCase());
      expect(events[0].args["failedRatio"]).to.be.eq(newRatio);
      expect(events[0].args["reason"]).to.be.eq("new ratio too low, not in threshold range");
      expect(await ratioFeed.getRatioFor(certificateToken.address)).to.be.eq(ratioBefore);
    });
    ...
    it("Reverts: only operator can", async function () {
      const ratioBefore = await ratioFeed.getRatioFor(certificateToken.address);
      const newRatio = ratioBefore.sub(1);
      await expect(ratioFeed.connect(signer1).updateRatioBatch([certificateToken.address], [newRatio]))
  ...
  describe("repairRatioFor(): sets specified value", function () {
    ...
    it("Decrease ratio", async function () {
      ...
      console.log(`Ratio before: ${ratioBefore}`);
      const newRatio = ratioBefore.sub(1);
      const tx = await ratioFeed.repairRatioFor(certificateToken.address, newRatio);
      const receipt = await tx.wait();
      const events = receipt.events?.filter((e) => {
        return e.event === "RatioUpdated";
      });
    ...
    it("Reverts: when new ratio = 0", async function () {
      await expect(ratioFeed.repairRatioFor(certificateToken.address, "0"))
        .to.be.revertedWith("ratio is zero");
    });

    it("Reverts: only governance can", async function () {
      await expect(ratioFeed.connect(signer1).repairRatioFor(certificateToken.address, randomBN(18)))
        .to.be.revertedWith("RatioFeed: only governance allowed");
    });
  ...
  describe("averagePercentageRate()", function () {
    ...
    it("Publish ratio for 8days", async function () {
      for (let i = 1; i < 8; i++) {
        ...
        await ratioFeed.connect(operator).updateRatioBatch([certificateToken.address], [newRatio]);
      }
    });

    it("Get apr", async function () {
      for (let i = 1; i < 8; i++) {
        const apr = await ratioFeed.averagePercentageRate(certificateToken.address, i);
        const aprPercent = apr.div(toBN("1000000000")).toNumber() / 1000000000;
