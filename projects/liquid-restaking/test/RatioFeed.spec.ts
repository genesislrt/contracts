...
const init = async () => {
    [governance, operator, signer1, signer2, token1, token2] =
...
describe('RatioFeed', function () {
    ...
    describe('updateRatio(): publishes ratio for listed addresses', function () {
        ...
        it('Publish ratio for the first time', async function () {
            ...
            await expect(
                ratioFeed.connect(operator).updateRatio(token1.address, _1E18)
            )
        ...
        it('Publish ratio within threshold', async function () {
            ...
            const ratioBefore = await ratioFeed.getRatio(token1.address);
            const threshold =
                (ratioBefore * (await ratioFeed.ratioThreshold())) /
                (await ratioFeed.MAX_THRESHOLD());
        ...
        it('Publish the same ratio', async function () {
            ...
            await expect(
                ratioFeed
                    .connect(operator)
                    .updateRatio(token1.address, ratioBefore)
            )
                .to.emit(ratioFeed, 'RatioUpdated')
        ...
        it('Reverts: only operator can', async function () {
            ...
            const newRatio = ratioBefore - 1n;
            await expect(
                ratioFeed.connect(signer1).updateRatio(token1.address, newRatio)
    ...
    describe('repairRatioFor(): sets specified value', function () {
        before(async function () {
            ...
            await ratioFeed
                .connect(operator)
                .updateRatio(token1.address, _1E18);
        });
        ...
        it('Reverts: only governance can', async function () {
            await expect(
                ratioFeed
                    .connect(signer1)
                    .repairRatio(token1.address, randomBN(18))
            ).to.be.revertedWithCustomError(ratioFeed, 'OnlyGovernanceAllowed');
        });
    });
