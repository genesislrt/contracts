...
describe('FeeCollector', function () {
    ...
    describe('Getter and Setter', () => {
        ...
        it('Reverts: new commission too big', async () => {
            await expect(
                collector.setCommission(10001)
            ).to.be.revertedWithCustomError(collector, 'CommissionNotInRange');
        });

        it('Reverts: when caller not governance', async () => {
            await expect(
                collector.connect(operator).setCommission(1)
            ).to.be.revertedWithCustomError(collector, 'OnlyGovernanceAllowed');
        });
    });
    ...
    describe('withdraw', () => {
        ...
        it('withdraw()', async () => {
            ...
            const fee =
                (amount * (await collector.commission())) /
                (await collector.MAX_COMMISSION());
            ...
            expect(contractBalanceBefore - contractBalanceAfter).to.be.eq(
                amount
            );
